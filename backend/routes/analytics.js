 ...
 * Get comprehensive financial overview with metrics
 */
...
router.get('/exchanges-disabled', authenticateToken, async (req, res) => {
  ...
  try {
    const options = {
      ...
      limit: Math.min(parseInt(req.query.limit) || 30, 100), // Max 100 per page
    ...
    console.log('📊 Fetching paginated exchanges with options:', options);
    
    const result = await analyticsService.getPaginatedExchanges(options);
    ...
    console.error('Paginated exchanges error:', error);
    res.status(500).json({
      success: false,
      error: error.message,
      timestamp: new Date().toISOString()
 ...
 * Get enhanced dashboard statistics (optimized for performance)
 */
router.get('/dashboard-stats', authenticateToken, async (req, res) => {
  try {
    ...
    const cacheKey = `dashboard_stats_${req.user?.id || 'global'}`;
    ...
    // Fetch data - run simpler queries in parallel
    const [overview, recentExchanges] = await Promise.all([
      analyticsService.getQuickOverview(), // Use optimized method
      analyticsService.getRecentExchanges(5) // Get just 5 recent exchanges
    ]);
    ...
    // Cache for 1 minute
    analyticsService.setCache(cacheKey, stats, 60000);
