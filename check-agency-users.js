...
async function checkAndCreateAgencyUsers() {
  try {
    ...
    if (agencyUsers.length === 0) {
      ...
      for (const agencyData of sampleAgencies) {
        try {
          ...
          const newUser = await databaseService.createUser({
            ...agencyData,
            password_hash: '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J/HS.iK8e' // password: agency123
          });
      ...
      console.log('🏢 Agency users after creation:', updatedAgencyUsers.length);
      
      if (updatedAgencyUsers.length > 0) {
        console.log('📋 Agency users:');
        updatedAgencyUsers.forEach(user => {
          console.log(`  - ${user.email} (${user.first_name} ${user.last_name}) - ${user.company || 'No company'}`);
        });
      }
    } else {
      console.log('📋 Existing agency users:');
