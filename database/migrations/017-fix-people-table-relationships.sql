-- Migration: Fix People Table Relationships
...
-- (exchanges should reference contact records for clients, user records for coordinators)

-- First, let's check and fix any exchanges with incorrect references
UPDATE exchanges e
...
INSERT INTO audit_logs (
    ...
    (SELECT id FROM people WHERE role = 'admin' AND is_user = true LIMIT 1),
    'MIGRATION_APPLIED',
    'DATABASE',
    jsonb_build_object(
        'migration', '017-fix-people-table-relationships',
        'description', 'Fixed relationships in unified people table',
        'changes', ARRAY[
            'Added contact_link_id to people table',
            'Linked user records to contact records',
            'Created get_user_exchanges function',
            'Added automatic linking trigger',
            'Created user_with_contact_info view',
            'Fixed exchange client references'
        ]
    ),
    NOW()
);
